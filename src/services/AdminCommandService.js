class AdminCommandService {
  ...
  constructor(adminId, guestService, productService, payloadParser) {
  ...
  handleAdminCommand(chatId, text) {
    if (!text.startsWith('/admin_') || chatId !== this.#adminId) return null;
    ...
    return { text: 'Команды: /admin_users, /admin_products, /admin_product_add, /admin_product_edit, /admin_product_delete' };
  }
  ...
  #handleDelete(text) {
    const id = Number(text.replace('/admin_product_delete ', '').trim());
    const deleted = Number.isInteger(id) && id > 0 ? this.#productService.removeProduct(id) : false;
    return { text: deleted ? `Карточка #${id} удалена.` : 'Не удалось удалить карточку. Проверьте id.' };
  }
}
